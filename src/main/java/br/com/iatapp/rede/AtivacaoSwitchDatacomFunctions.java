...
import br.com.iatapp.commons.SwitchDatacomCommons;
...
public class AtivacaoSwitchDatacomFunctions {
	 ...
	 */
	public AtivacaoSwitchDatacomFunctions(RedeIpFunctions redeIpFunctions, ConfigSwitchIdDomain configSwitchIdDomain, List<ConfigSwitchScriptDomain> lstScripts) {
		...
		try {
			...
			ExceptionLogger.record(ExceptionUtils.getStackTrace(e),
					RedeHelper.retornaInfoProcedimento(AtivacaoSwitchDatacomFunctions.class.getName(), configSwitchIdDomain.getHostnameSwa(), "AtivacaoSwitchDatacomFunctions"));
		}
	}
	...
	public void iniciaProcedimentoPreCheckConfiguracao() {
		...
		if(configSwitchIdDomain.getTipoInterface().equals("ge"))
			configSwitchIdDomain.setTipoInterface("gigabit-ethernet");
		
		SwitchDatacomCommons switchDatacomCommons = new SwitchDatacomCommons(redeIpFunctions, configSwitchIdDomain);
		RetornoModel retornoModel;
		
		// FIND_USER_CONFIGURE
		
		retornoModel = switchDatacomCommons.findUserConfigure(hostname.toLowerCase(), configSwitchIdDomain.getIpSwa());
		...
		// Limpa Log
		redeIpFunctions.limpaLog(RedeHelper.retornaInfoProcedimento(AtivacaoSwitchDatacomFunctions.class.getName(), 
				hostname, "FIND_USER_CONFIGURE"));
		
		// CHECK_STATUS_INTERFACE
		
		retornoModel = switchDatacomCommons.checkStatusInterface(hostname.toLowerCase(), configSwitchIdDomain.getTipoInterface(), configSwitchIdDomain.getPortaCliente());
		gravaLogResultado.record(configSwitchIdDomain.getTipoTeste().getCodigo(), 
		...
		if(retornoModel.isResultado()) {			
			retornoModel = switchDatacomCommons.iniciaProcedimentoConfiguracao(configSwitchIdDomain, hostname.toLowerCase(), lstScripts);
			gravaLogResultado.record(configSwitchIdDomain.getTipoTeste().getCodigo(), 
		...
		if(configSwitchIdDomain.getIdModeloEquipamento() == ModelosEquipamentosEnum.DATACOM.getCodigo()) {
			retornoModel = switchDatacomCommons.checkVlanGerencia(hostname.toLowerCase(), configSwitchIdDomain.getVlanGerenciaSwa());
			gravaLogResultado.record(configSwitchIdDomain.getTipoTeste().getCodigo(), 
		...
		retornoModel = switchDatacomCommons.checkVlanTunelServico(hostname.toLowerCase(), configSwitchIdDomain);
		gravaLogResultado.record(configSwitchIdDomain.getTipoTeste().getCodigo(), 
		...
		retornoModel = switchDatacomCommons.checkConfiguracaoInterface(hostname.toLowerCase(), configSwitchIdDomain);
		gravaLogResultado.record(configSwitchIdDomain.getTipoTeste().getCodigo(), 
		...
		if(configSwitchIdDomain.getIdModeloEquipamento() == ModelosEquipamentosEnum.DATACOM.getCodigo()) {
			retornoModel = switchDatacomCommons.checkValidacaoVlanTable(hostname.toLowerCase(), configSwitchIdDomain);
			gravaLogResultado.record(configSwitchIdDomain.getTipoTeste().getCodigo(), 
