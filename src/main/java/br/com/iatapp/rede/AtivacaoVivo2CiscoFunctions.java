...
import br.com.iatapp.commons.Vivo2CiscoCommons;
...
public class AtivacaoVivo2CiscoFunctions {
	...
	public AtivacaoVivo2CiscoFunctions(RedeIpFunctions redeIpFunctions) {
	 ...
	 * @param vivo2IdDomain
	 */
	public AtivacaoVivo2CiscoFunctions(RedeIpFunctions redeIpFunctions, Vivo2IdDomain vivo2IdDomain, List<Vivo2ScriptDomain> lstScripts) {
		...
		try {
			...
			ExceptionLogger.record(ExceptionUtils.getStackTrace(e),
					RedeHelper.retornaInfoProcedimento(AtivacaoVivo2CiscoFunctions.class.getName(), vivo2IdDomain.getIdTbs(), "AtivacaoVivo2CiscoFunctions"));
		}
	}
	...
	public void iniciaProcedimento() {
		...
		Vivo2CiscoCommons vivo2CiscoCommons = new Vivo2CiscoCommons(redeIpFunctions, vivo2IdDomain);
		RetornoModel retornoModel;		
		
		// CHECK_VALIDACAO_IPS_LIVRES
		
		if(vivo2IdDomain.getServico().equals("SIP")) {
			retornoModel = vivo2CiscoCommons.checkVrfSipExiste();
		...
		retornoModel = vivo2CiscoCommons.checkIpsLivres();
		gravaLogResultado.record(vivo2IdDomain.getTipoTeste().getCodigo(), 
		...
		// Limpa Log
		redeIpFunctions.limpaLog(RedeHelper.retornaInfoProcedimento(AtivacaoVivo2CiscoFunctions.class.getName(), 
		...
		retornoModel = vivo2CiscoCommons.checkInterfaceConfiguracao();
		gravaLogResultado.record(vivo2IdDomain.getTipoTeste().getCodigo(), 
		...
		if(retornoModel.isResultado()) {			
			retornoModel = vivo2CiscoCommons.iniciaProcedimentoConfiguracao(lstScripts);
			gravaLogResultado.record(vivo2IdDomain.getTipoTeste().getCodigo(), 
