...
public class SwitchDatacomCommons {
	
	RedeIpFunctions redeIpFunctions;
	ConfigSwitchIdDomain configSwitchIdDomain;
	
	public SwitchDatacomCommons(RedeIpFunctions redeIpFunctions, ConfigSwitchIdDomain configSwitchIdDomain) {
	...
	public RetornoModel findUserConfigure(String hostname, String ipSwa) {
		...
		try {			
			String comando = String.format("config");
			String retorno = redeIpFunctions.enviarComandoAvailableMore(comando, new String[] {hostname + "#", hostname + ">", ")#"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "findUserConfigure"));
			...
			if (StringUtils.isBlank(retorno) 
				...
				redeIpFunctions.fecharConexaoEquipamento(RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), configSwitchIdDomain.getIdVantive(), "findUserConfigure"));
				...
				for(int c = 0; c < array.length(); c++) {
					...
					if(redeIpFunctions.conectarEquipamentoSwitch(configSwitchIdDomain.getIpSwa(), json.getString("login"), json.getString("senha"),
						...
						comando = String.format("config");
						retorno = redeIpFunctions.enviarComandoAvailableMore(comando, new String[] {hostname + "#", hostname + ">", ")#"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
								RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "findUserConfigure"));
			...
			retorno = redeIpFunctions.enviarComandoAvailableMore(comando, new String[] {hostname + "#", hostname + ">", ")#"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "iniciaProcedimentoConfiguracao"));
			...
			ExceptionLogger.record(ExceptionUtils.getStackTrace(e),
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), configSwitchIdDomain.getHostnameSwa(), "checkVlanGerencia"));			
	...
	public RetornoModel checkStatusInterface(String hostname, String tipoInterface, String portaCliente) {
		...
		try {
			String comando = String.format("show interfaces status %s %s", tipoInterface, portaCliente);
			String retorno = redeIpFunctions.enviarComandoAvailableMore(comando, new String[] {hostname + "#", hostname + ">"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "checkStatusInterface"));
			
			redeIpFunctions.getLog().append(StringUtils.isNotBlank(retorno) ? retorno : "");
			
			if (StringUtils.isBlank(retorno) || StringUtils.containsIgnoreCase(retorno, "syntax error:")) {
				
				comando = String.format("show interface %s %s", tipoInterface, portaCliente);
				retorno = redeIpFunctions.enviarComandoAvailableMore(comando, new String[] {hostname + "#", hostname + ">"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
						RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "checkStatusInterface"));
				redeIpFunctions.getLog().append(StringUtils.isNotBlank(retorno) ? retorno : "");
			...
			for(int c = 0; c < linhasComando.length; c++) {
				...
				if(StringUtils.containsIgnoreCase(linha, "Link status")) {
					String status = StringUtils.substringAfter(linha, ":").trim();
					if(StringUtils.equalsIgnoreCase(status, "Down")) {
						retornoModel = new RetornoModel();
						retornoModel.setCodigo(ResultadosStatus.OK.getCodigo());
						retornoModel.setResultado(true);
						retornoModel.setRetorno(String.format("Check Ok. Status = %s", status));
						retornoModel.setLog(StringHelper.removerCodigoCores(redeIpFunctions.getLog().toString()));			
						return retornoModel;
					}
					...
					retornoModel.setCodigo(ResultadosStatus.FALHOU.getCodigo());
					retornoModel.setResultado(false);
					retornoModel.setRetorno(String.format("Check Falhou. Status = %s", status));
					retornoModel.setLog(StringHelper.removerCodigoCores(redeIpFunctions.getLog().toString()));			
					return retornoModel;
				}
			}
	...
	public RetornoModel iniciaProcedimentoConfiguracao(ConfigSwitchIdDomain configSwitchIdDomain, String hostname, List<ConfigSwitchScriptDomain> lstScripts) {
		...
		try {
			...
			for(ConfigSwitchScriptDomain configSwitchScriptDomain: lstScripts) {
				...
				for(int c = 0; c < linhasComando.length; c++) {					
					String linha = linhasComando[c].trim();
					retorno = redeIpFunctions.enviarComandoAvailableMore(linha, new String[] {hostname + "#", hostname + ">", ")#"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
							RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "iniciaProcedimentoConfiguracao"));
			...
			comando = String.format("exit");
			retorno = redeIpFunctions.enviarComandoAvailableMore(comando, new String[] {hostname + "#", hostname + ">", ")#", "[yes/no/CANCEL]"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "iniciaProcedimentoConfiguracao"));
	...
	public RetornoModel checkVlanGerencia(String hostname, String vlanGerencia) {
		...
		try {
			...
			String retorno = redeIpFunctions.enviarComandoAvailableMore(comando, new String[] {hostname + "#", hostname + ">"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "checkVlanGerencia"));
	...
	public RetornoModel checkVlanTunelServico(String hostname, ConfigSwitchIdDomain configSwitchIdDomain) {
		...
		try {
			...
			String retorno = redeIpFunctions.enviarComandoAvailableMore(comando, new String[] {hostname + "#", hostname + ">"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "checkVlanTunelServico"));
			...
			ExceptionLogger.record(ExceptionUtils.getStackTrace(e),
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), configSwitchIdDomain.getHostnameSwa(), "checkVlanTunelServico"));			
	...
	public RetornoModel checkConfiguracaoInterface(String hostname, ConfigSwitchIdDomain configSwitchIdDomain) {
		...
		try {
			...
			String retorno = redeIpFunctions.enviarComandoAvailableMore(comando, new String[] {hostname + "#", hostname + ">"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "checkConfiguracaoInterface"));
			...
			ExceptionLogger.record(ExceptionUtils.getStackTrace(e),
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), configSwitchIdDomain.getHostnameSwa(), "checkConfiguracaoInterface"));			
	...
	public RetornoModel checkValidacaoVlanTable(String hostname, ConfigSwitchIdDomain configSwitchIdDomain) {
		...
		try {
			...
			String retorno = redeIpFunctions.enviarComandoAvailableMore(comando, new String[] {hostname + "#", hostname + ">"}, new String[] {"-- more --", "--More--", "-- More --"}, SimpleTelnet.SPACE,
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), hostname, "checkValidacaoVlanTable"));
			...
			ExceptionLogger.record(ExceptionUtils.getStackTrace(e),
					RedeHelper.retornaInfoProcedimento(SwitchDatacomCommons.class.getName(), configSwitchIdDomain.getHostnameSwa(), "checkValidacaoVlanTable"));			
